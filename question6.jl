...
function resoudre_UFLP_relaxation(n, tabX, tabY, p)
    ...
    @constraint(model, [i=1:n], sum(dist(tabX[i], tabY[i], tabX[j], tabY[j]) * x[i, j] for j in 1:n) <= z)
    ...
    diag_vals = zeros(n)     
    for i in 1:n
        diag_vals[i] = x_values[i, i]
    end

    indices_tries = sortperm(diag_vals, rev=true)
    ...
    println("Distance maximale minimisée (z): ", objective_value(model))
    return objective_value(model), S, x_values
end
...
p = 10
z,S,x = resoudre_UFLP_relaxation(n, tabX, tabY, p)
