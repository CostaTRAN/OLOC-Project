...
function Lit_fichier_CSV(nom_fichier, tabX, tabY, tab_departement)
    n=0
    open(nom_fichier) do fic
  
          for (i,line) in enumerate(eachline(fic)) 
                lg = split(line," ")      # découpe la ligne suivant les espaces et crée un tableau 
                push!(tab_departement,parse(Int,lg[2]))
                push!(tabX,parse(Float64,lg[5]))
                push!(tabY,parse(Float64,lg[6]))
                n+=1
          end
    end
    println("Le fichier contient ",n, " villes")
...
# Fonction Dessine instance
function Dessine_CSV(nom_fichier, n, tabX, tabY, tab_departement, S)
    
    X=Float64[]
    Y=Float64[]

    nom_fichier_en_deux_morceaux = split(nom_fichier,".") # découpe le nom du fichier d'entrée sans l'extension
    nom_fichier_avec_pdf_a_la_fin= nom_fichier_en_deux_morceaux[1]*"_sol.pdf"
...
function genere_solution_departement(n, tab_departement, p)
    if p > n
    ...
    i = 1
    while i < n && p > 0
        if(dept != tab_departement[i])
            dept = tab_departement[i]
            S[i] = 1
            p -= 1
        end
        i += 1
...
tabX = Float64[]
tabY = Float64[]
tab_departement = Int64[]

# Affichage des résultats
p = 95
n = Lit_fichier_CSV("villes_france_metro.csv", tabX, tabY, tab_departement)
S = genere_solution_departement(n, tab_departement, p)
Dessine_CSV("departement", n, tabX, tabY, tab_departement, S)
